	...
	private void _updateScene() {
		...
		for(Vector2 vec: pillars) {
			...
			if(vec.y == 1)
				...
				obstacleRect.set(vec.x+20, WND_HEIGHT-PILLAR_HEIGHT, PILLAR_WIDTH, PILLAR_HEIGHT);
	...
	private void _drawScene() {
		...
		for(Vector2 vec: pillars) {
			if(vec.y == 1)
				...
				batch.draw(pillarDown, vec.x, WND_HEIGHT-PILLAR_HEIGHT, PILLAR_WIDTH, PILLAR_HEIGHT);
		}
